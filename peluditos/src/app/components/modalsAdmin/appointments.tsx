...
export const AddAppoinments: React.FC<appointmentsProps> = ({ show, handleClose, data, action, updateData }) => {
    ...
    const onSubmit: SubmitHandler<data> = async (data) => {
        ...
        const resp = 201//await createAppointment(appointment);
    ...
    const loadAvailableDays = async (id_speciality: number) => {
        const calendar = ['Lunes', 'Viernes'] //await getAvailableDays(id_speciality);
    ...
    const loadAvailableTimes = async (id_service: number, day: string) => {
        const times = ['10:00'] //await getAvailableTimes({ id_service: id_service, day: day });
    ...
    const loadSpecialties = async () => {
        const resp = [ { id: '1', speciality: 'Cardiología' }, { id: '2', speciality: 'Dermatología' }] //await getSpecialtiesWhitProf();
        setSpecialties(resp);
    }

    const loadServices = async (id_speciality: any) => {
        const resp = [{ id: 1, service: 'Consulta general', duration: 30 }] //await getServicesBySpeciliaty(id_speciality);
        setServices(resp);
    ...
    const findClient = async (dni: number) => {
        const resp = {dni: 31094435, id_user:1, statusCode:200, message:'ok'}//await getClientByDni(dni);
        if (resp.statusCode) {
